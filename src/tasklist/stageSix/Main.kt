...
fun editATask(tasks: MutableList<Task>) {
    ...
    when (getValidTaskField()) {
        ...
        "task" -> {
            ...
            tasks[index] =
                Task(oldTask.priority, oldTask.isoDateTime, newTaskLines)
        }
    }
...
fun getValidPriority(): Priority {
    while (true) {
        val validPries = tasklist.stageFive.Priority.values().map { it.string }
